...
class FullRadarProviderTests(unittest.TestCase):
    ...
    def test_project_browser_forwards_public_filters_and_requires_snapshot(self) -> None:
        transport = RecordingTransport(
            [
                response(
    ...
    def test_skills_browser_forwards_search_category_and_pagination(self) -> None:
        transport = RecordingTransport(
            [
                response(
        ...
        output = provider.browse_radar_skills(
            {"locale": "en", "query": "review", "category": "coding", "limit": 5, "offset": 0}
        )

        self.assertEqual(
            transport.calls[0]["query"],
            {"lang": "en", "limit": 5, "offset": 0, "q": "review", "category": "coding"},
        )
